...
import ProductItemParts from '../MultiUse/ProductItemParts'
...
const ComputerPartSection = () => {
  const { ComputerParts } = useContext(ShopContext)
    ...
    useEffect(()=>{
      const bestProduct = ComputerParts.filter((e)=>(e.bestSeller))  // here if the value is true the filter will save all the true values in bestProduct and otherwise it will not store the value.
  ...
  return (
    <div className='my-2 mb-10 py-8 pt-2 px-4 rounded-md shadow-md bg-white '>
      <div className='text-center text-2xl py-8'>
        <Title title1="COMPUTER PARTS" title2="SECTION"/>
        <p className="w-4/5 m-auto text-xs sm:text-sm md:text-base text-gray-600">
          Browse our Computer Parts section for high-quality components that power your PC. From processors and graphics cards to storage and cooling solutions, find everything you need to build or upgrade your system. 
        </p>
      </div>
