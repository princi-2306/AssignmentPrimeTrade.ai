...
import Filters from "../Components/ProductsUse/Filters";
import Title from "../Components/MultiUse/Title";
import ProductItem from "../Components/MultiUse/ProductItem";
...
const ProductsLaptops = () => {
  ...
  const handleFilteredObject = (filteredObject) => {
    ...
    console.log(filteredProducts, filteredObject);
    ...
    if (filteredObject.type && filteredObject.type.length > 0) {
      filteredProducts = filteredProducts.filter((product) => {
        // Extract and split the type array correctly
        const typeArray = product.type[0].split(',').map(item => item.trim());
    
        // Check if any of the filteredObject types exist in the product types
        return filteredObject.type.some((type) => typeArray.includes(type));
      });
    }
  ...
  useEffect(() => {
    let sortedProducts = [...LaptopProducts];
  ...
  return (
    <div>
      <div className="mt-4 flex flex-col md:flex-row my-8">
        {/* Filters Section */}
        <Filters sendFilteredObject={handleFilteredObject} />
