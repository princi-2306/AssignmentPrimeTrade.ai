...
import ProductItemPrinters from "../Components/MultiUse/ProductItemPrinters";


const ProductsPrintersAndCctvID = () => {
  const { ProductsPrintersAndCctvID } = useParams();
  const { PrintersAndCCTVs, currency } = useContext(ShopContext);
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      const selectedProduct = PrintersAndCCTVs.find((e) => e._id === ProductsPrintersAndCctvID);
      ...
      if (selectedProduct) {
        ...
        const related = PrintersAndCCTVs.filter((product) => 
          // Exclude the current product and match at least one type
          product._id !== ProductsPrintersAndCctvID && product.type.some((type) => selectedProduct.type.includes(type))
        );
    ...
    setValueRating(4.6);
    fetchProductData();
  }, [PrintersAndCCTVs, ProductsPrintersAndCctvID]);
...
export default ProductsPrintersAndCctvID;