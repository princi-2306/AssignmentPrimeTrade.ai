...
const ProductsLaptopsID = () => {
  const { ProductsLaptopsID } = useParams();
  const { LaptopProducts, currency } = useContext(ShopContext);
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      const selectedProduct = LaptopProducts.find((e) => e._id === ProductsLaptopsID);
      const specifications = {
        "RAM": selectedProduct.ram, 
        "Storage":selectedProduct.storage, 
        "Processor":selectedProduct.processor, 
        "Graphics Card":selectedProduct.graphicsCard, 
      ...
      if (selectedProduct) {
        ...
        const related = LaptopProducts.filter((product) => 
          // Exclude the current product and match at least one type
          product._id !== ProductsLaptopsID && product.type.some((type) => selectedProduct.type.includes(type))
        );
    ...
    setValueRating(4.6);
    fetchProductData();
  }, [LaptopProducts, ProductsLaptopsID]);
...
export default ProductsLaptopsID;