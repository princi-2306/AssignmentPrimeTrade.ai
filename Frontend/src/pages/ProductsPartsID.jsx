...
const ProductsPartsID = () => {
  const { ProductsPartsID } = useParams();
  const { ComputerParts, currency } = useContext(ShopContext);
  ...
  useEffect(() => {
    const fetchProductData = async () => {
      const selectedProduct = ComputerParts.find((e) => e._id === ProductsPartsID);
      ...
      if (selectedProduct) {
        ...
        const related = ComputerParts.filter((product) => 
          // Exclude the current product and match at least one type
          product._id !== ProductsPartsID && product.type.some((type) => selectedProduct.type.includes(type))
        );
...
export default ProductsPartsID;